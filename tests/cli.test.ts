...
describe('CLI', () => {
  ...
  describe('build command', () => {
    ...
    it('should include project files in build output', async () => {
      ...
      expect(fs.existsSync(path.join(projectDir, 'TestBoard.kicad_pro'))).toBe(true);
      expect(fs.existsSync(path.join(projectDir, 'TestBoard.kicad_sch'))).toBe(true);
      expect(fs.existsSync(path.join(projectDir, 'TestBoard.kicad_pcb'))).toBe(true);
    });
    ...
    it('should have valid project-index.json structure', () => {
      const indexPath = path.join(buildOutputDir, 'project-index.json');
      const projectIndex: ProjectIndex = JSON.parse(fs.readFileSync(indexPath, 'utf-8'));

      // Top-level fields
      expect(projectIndex.version).toMatch(/^\d+\.\d+\.\d+$/);
      expect(new Date(projectIndex.generatedAt).toString()).not.toBe('Invalid Date');
    ...
    it('should have valid paths in project-index.json', () => {
      ...
      for (const project of projectIndex.projects) {
        // Project file exists
        expect(fs.existsSync(path.join(buildOutputDir, project.projectFile))).toBe(true);
        ...
        // PCB exists
        if (project.pcb) {
          expect(fs.existsSync(path.join(buildOutputDir, project.pcb))).toBe(true);
        }

        // Zip exists
        if (project.zip) {
          expect(fs.existsSync(path.join(buildOutputDir, project.zip))).toBe(true);
        }
      }
    });
