...
describe("Indexer", () => {
  ...
  describe("indexer-utils", () => {
    ...
    it("should initialize paths correctly", async () => {
      const { initializePaths, getRootDir, getOutputDir, getKisiteRootDir } =
        await import("../src/indexer/indexer-utils.js");
    ...
    it("should get project directories from config", async () => {
      const { loadConfig, getProjectDirs, initializePaths } =
        await import("../src/indexer/indexer-utils.js");
  ...
  describe("scan-projects", () => {
    it("should find KiCad project files", async () => {
      const { initializePaths, loadConfig, getProjectDirs, getTrackedFiles } =
        await import("../src/indexer/indexer-utils.js");
      ...
      const proFiles = Array.from(trackedFiles).filter((f) =>
    ...
    it("should find schematic files", async () => {
      ...
      const schFiles = Array.from(trackedFiles).filter((f) =>
    ...
    it("should find PCB files", async () => {
      ...
      const pcbFiles = Array.from(trackedFiles).filter((f) =>
